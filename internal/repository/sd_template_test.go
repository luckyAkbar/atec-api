...
func TestSDTemplateRepository_UndoDelete(t *testing.T) {
	kit, closer := common.InitializeRepoTestKit(t)
	defer closer()
	...
	tests := []common.TestStructure{
		{
			...
			MockFn: func() {
				mock.ExpectBegin()
				mock.ExpectQuery(`UPDATE "test_templates" SET`).WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg(), te.ID).WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(te.ID))
				mock.ExpectCommit()
			},
			Run: func() {
				_, err := repo.UndoDelete(ctx, te.ID)
				assert.NoError(t, err)
			},
		},
		{
			...
			MockFn: func() {
				mock.ExpectBegin()
				mock.ExpectQuery(`UPDATE "test_templates" SET`).WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg(), te.ID).WillReturnError(errors.New("err db"))
				mock.ExpectCommit()
			},
			Run: func() {
				_, err := repo.UndoDelete(ctx, te.ID)
